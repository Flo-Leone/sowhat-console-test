...
const barColors = [
  "hsl(205, 52%, 55%)", "hsl(180, 70%, 45%)", "hsl(330, 80%, 55%)",
  "hsl(38, 100%, 55%)", "hsl(44, 100%, 67%)", "hsl(80, 50%, 50%)",
  "hsl(241, 54%, 62%)", "hsl(205, 70%, 55%)", "hsl(330, 60%, 55%)",
  "hsl(18, 100%, 69%)", "hsl(0, 0%, 55%)",
];

const EmbedAnalytiquePage = () => {
    ...
    <div className="p-6 lg:p-8 space-y-6 animate-fade-in bg-transparent">
      ...
      <div className="flex items-center gap-4 flex-wrap">
        <div className="flex items-center gap-2">
          ...
          <Select value={selectedOffre} onValueChange={setSelectedOffre}>
            <SelectTrigger className="w-52"><SelectValue placeholder="Toutes les offres" /></SelectTrigger>
            <SelectContent>
      ...
      <div className="bg-card rounded-xl overflow-hidden">
        <div className="px-6 py-4 border-b border-border/50">
          ...
          <p className="text-sm text-muted-foreground mt-0.5">Nombre de candidats ayant atteint chaque étape du chat</p>
        </div>
        <div className="p-6">
          <div className="h-[420px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={dataWithDropoff.filter(d => !d.excludeFromChart)} margin={{ top: 20, right: 20, bottom: 60, left: 20 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="hsl(40 15% 88%)" vertical={false} />
                <XAxis dataKey="question" tick={{ fontSize: 11, fill: "hsl(0 0% 45%)" }} angle={-35} textAnchor="end" height={80} interval={0} />
                <YAxis tick={{ fontSize: 12, fill: "hsl(0 0% 45%)" }} tickFormatter={(v: number) => v >= 1000 ? `${(v / 1000).toFixed(0)}K` : v.toString()} />
                <Tooltip formatter={(value: number) => [value.toLocaleString("fr-FR"), "Candidats"]} labelFormatter={(label: string) => label} contentStyle={{ borderRadius: "8px", border: "1px solid hsl(40 15% 88%)", fontSize: "13px" }} />
                <Bar dataKey="candidats" radius={[4, 4, 0, 0]} maxBarSize={60}>
                  {dataWithDropoff.filter(d => !d.excludeFromChart).map((_, index) => (
                    <Cell key={index} fill={barColors[index % barColors.length]} />
                  ))}
                  <LabelList dataKey="candidats" position="top" formatter={(v: number) => v >= 1000 ? `${(v / 1000).toFixed(1)}K` : v} style={{ fontSize: 11, fill: "hsl(0 0% 45%)" }} />
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </div>
      ...
      <div className="bg-card rounded-xl overflow-hidden">
        <div className="px-6 py-4 border-b border-border/50">
          ...
          <p className="text-sm text-muted-foreground mt-0.5">Taux d'abandon entre chaque étape du tunnel</p>
        </div>
                ...
                return (
                  <tr key={index}>
                    ...
                    <td className="text-right">
                      {index === 0 ? "—" : <span className="text-[hsl(var(--destructive))]">-{item.dropoff.toLocaleString("fr-FR")}</span>}
                    </td>
                    <td className="text-right">
                      {index === 0 ? "—" : (
...
export default EmbedAnalytiquePage;
