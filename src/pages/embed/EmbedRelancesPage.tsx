    ...
    <div className="p-6 lg:p-8 space-y-6 animate-fade-in bg-transparent">
      ...
      <div className="bg-card rounded-xl overflow-hidden">
        <div className="px-6 py-4 border-b border-border/50">
          <h2>Déclencheur</h2>
          <p className="text-sm text-muted-foreground mt-0.5">Événement qui déclenche la relance automatique</p>
        </div>
        <div className="p-6">
          <div className="space-y-1.5">
            <label className="form-label">Déclencheur</label>
            <Select value={declencheur} onValueChange={setDeclencheur}>
              <SelectTrigger className="w-full max-w-sm"><SelectValue placeholder="Sélectionner un déclencheur" /></SelectTrigger>
              <SelectContent>
      ...
      <div className="bg-card rounded-xl overflow-hidden">
        <div className="px-6 py-4 border-b border-border/50 flex items-center justify-between">
          <div>
            <h2>Conditions</h2>
            <p className="text-sm text-muted-foreground mt-0.5">Règles à remplir pour déclencher l'action</p>
          </div>
          <button type="button" className="btn-secondary btn-sm gap-1.5" onClick={addCondition}>
            <Plus className="h-4 w-4" /> Ajouter condition
          </button>
        </div>
        <div className="p-6 space-y-4">
          {conditions.map((condition) => (
            <div key={condition.id} className="grid grid-cols-[1fr_1fr_1fr_1fr_auto] gap-4 items-end">
              <div className="space-y-1.5">
                <label className="form-label">Type</label>
                <Select value={condition.type} onValueChange={(v) => updateCondition(condition.id, "type", v)}>
              ...
              <div className="space-y-1.5">
                ...
                <Select value={condition.volumeType} onValueChange={(v) => updateCondition(condition.id, "volumeType", v)}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
              ...
              <div className="space-y-1.5">
                ...
                <Select value={condition.comparison} onValueChange={(v) => updateCondition(condition.id, "comparison", v)}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
              ...
              <div className="space-y-1.5">
                ...
                <Input type="number" value={condition.quantity} onChange={(e) => updateCondition(condition.id, "quantity", parseInt(e.target.value) || 0)} className="border-b-2 border-b-[hsl(var(--golden-pollen))]" />
              </div>
              <button onClick={() => removeCondition(condition.id)} className="h-10 w-10 flex items-center justify-center rounded-md hover:bg-muted transition-colors text-muted-foreground hover:text-foreground" title="Supprimer la condition">
      ...
      <div className="bg-card rounded-xl overflow-hidden">
        <div className="px-6 py-4 border-b border-border/50">
          <h2>Action</h2>
          <p className="text-sm text-muted-foreground mt-0.5">Modèle et canaux de communication pour la relance</p>
        </div>
        <div className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              { label: "Action e-mail", value: actionEmail, onChange: setActionEmail },
              { label: "Action SMS", value: actionSms, onChange: setActionSms },
              { label: "Action WhatsApp", value: actionWhatsapp, onChange: setActionWhatsapp },
              { label: "Action RCS", value: actionMessenger, onChange: setActionMessenger },
...
export default EmbedRelancesPage;
